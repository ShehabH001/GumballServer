...
import { generateToken, verifyToken } from "../services/access_token/accessTokenService.mjs";
...
import { isOTPExpired } from "../services/verification/by_email/utils/OTPHelper.mjs";
...
export const register = async (req, res) => {
  try {
    ...
    const userExists = await ActualUser.checkIfUserExists(user);
    if (userExists) {
      return res.status(400).json({ message: "User already exists" });
    }
    // check if valid to store the user in temp_user table
    const temp_user = await TempUser.storeUser(user); // store the user in temp_user table
    const user_otp = await OTP.getOTP(temp_user.email);
    if (!user_otp || (user_otp && isOTPExpired(user_otp.updated_at)))
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    await ActualUser.createUser(tempUser); // store the user in user table
...
export const login = async (req, res) => {
  try {
    ...
    const user = await ActualUser.login(req.body.email, req.body.password, req.body.registration_type);
    const token = generateToken(user.id); // check
    res
      .status(200)
      .set({
...
export const getCurrentUser = async (req, res) => {
  try {
    ...
    console.log("Actual user data:", actual_user);
...
export const googleAuthentication = async (req, res) => {
  try {
    ...
    const token = generateToken(actualUser.id);
    console.log(verifyToken(token)); // Verify the token to ensure it's valid
