...
class ActualUser {
  ...
  static async getUserByEmail(email) {
    ...
    static async checkIfUserExists(user) {
    const query = `SELECT * FROM ${this.#tableName} WHERE ${this.#columns.get(
      "email"
    )} = $1 AND ${this.#columns.get( "registration_type")} = $2`;
    const values = [user.email, user.registration_type];
    const result = await darwinPool.query(query, values);
  ...
  static async login(email, password, registration_type) {
    const query = `SELECT * FROM ${this.#tableName} WHERE  ${this.#columns.get(
      "email"
    )} = $1 AND  ${this.#columns.get("password")} = $2 AND ${this.#columns.get("registration_type")} = $3`;
  ...
  static async createUser(user) {
    const query = `
      ...
      "email"
    )}, ${this.#columns.get("country_code")}, ${this.#columns.get(
      "mobile"
    )},${this.#columns.get("password")} ,${this.#columns.get(
      "registration_type"
    )} , ${this.#columns.get("customer_code")})
  ...
  static async storeActualUserIfNotExists(user, action) {
    const existingUser = await this.getUserByEmail(user.email);
    if (
      existingUser &&
      existingUser.registration_type !== user.registration_type
    ) {
      throw new Error();
    }
    if (existingUser) {
